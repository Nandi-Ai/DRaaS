...
def restart_services(service_names):
    for name in service_names:
        try:
            ...
            logging.info(f"restart_producer. Service {name} restarted successfully.")
        except subprocess.CalledProcessError as err:
            logging.error(f"restart_producer. Failed to restart service {name}. Error: {err}")
        except Exception as err:
            logging.error(f"restart_producer. An unexpected error occurred while restarting service {name}. Error: {err}")
